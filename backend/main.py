...
from sqlalchemy import DateTime, Float, ForeignKey, String, create_engine, delete, select
...
def user_dict(u): return {'id':u.id,'company':u.company,'firstName':u.first_name,'lastName':u.last_name,'fullName':f'{u.first_name} {u.last_name}','email':u.email,'phone':u.phone,'slug':u.slug}
...
def register(data:RegisterInput,db:Session=Depends(get_db)):
    ...
    u=User(company=data.company.strip(),first_name=data.firstName.strip(),last_name=data.lastName.strip(),email=email,phone=data.phone.strip(),password_hash=hash_password(data.password),slug=slug)
    db.add(u); db.commit(); db.refresh(u); return issue_token(db,u)
...
def public_menu(slug:str,db:Session=Depends(get_db)):
    ...
    return {'slug':slug,'business':u.company,'stocks':[stock_dict(s) for s in items]}
